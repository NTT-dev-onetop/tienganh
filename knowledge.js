...
export function initKnowledge(){
  ...
  if(stop)stop();
  const q=collection(db,'knowledge');
  stop=onSnapshot(q,snap=>{
    const rows=[];snap.forEach(d=>{const x={id:d.id,...d.data()};const pub=x.published===true||x.published===1||['true','published','public'].includes(String(x.published??'').toLowerCase())||['published','public'].includes(String(x.status??x.visibility??'').toLowerCase());if(pub)rows.push(x)});
  ...
  },e=>{console.error('Lỗi tải kiến thức:',e);root.innerHTML='<div class="alert alert-danger">Không tải được kho kiến thức. Hãy kiểm tra Firestore Rules/deploy.</div>'});
}
