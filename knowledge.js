import{collection,query,onSnapshot}from"https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js";
...
export function initKnowledge(){
  ...
  stop=onSnapshot(q,snap=>{
    ...
    const publishedRows=rows.filter(x=>x.published===true);
    publishedRows.sort((a,b)=>String(b.updatedAt?.seconds||b.createdAt?.seconds||0).localeCompare(String(a.updatedAt?.seconds||a.createdAt?.seconds||0)));
    root.innerHTML=publishedRows.length?publishedRows.map(k=>`<article class="knowledge-card"><div class="d-flex justify-content-between gap-2 flex-wrap"><div><span class="tag">${esc(k.category||'Kiến thức')}</span>${k.unit?` <span class="tag">${esc(k.unit)}</span>`:''}</div><small class="muted">${esc(k.authorName||'Giáo viên')}</small></div><h3>${esc(k.title||'Không có tiêu đề')}</h3>${k.content?`<div class="knowledge-block"><b>Lý thuyết</b><div>${text(k.content)}</div></div>`:''}${k.examples?`<div class="knowledge-block"><b>Ví dụ</b><div>${text(k.examples)}</div></div>`:''}${k.notes?`<div class="knowledge-note"><b>💡 Ghi chú</b><div>${text(k.notes)}</div></div>`:''}${k.exercises?`<div class="knowledge-exercises"><b>📝 Bài tập</b><div>${text(k.exercises)}</div></div>`:''}</article>`).join(''):'<div class="empty"><div>📚</div><h3>Chưa có kiến thức mới</h3><p>Giáo viên sẽ đăng bài tại Dashboard. Khi đăng xong, nội dung xuất hiện ở đây.</p></div>';
