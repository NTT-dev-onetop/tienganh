...
import{loadAdminForUser,stopAdminListeners}from"./admin.js";
import{initKnowledge,stopKnowledge}from"./knowledge.js";
import{initTeacherContent,stopTeacherContent}from"./teacher-content.js";
...
function page(id){
  ...
  const updatePage=()=>{
    ...
    if(id==='home')renderHome();if(id==='vocab')renderVocab();if(id==='grammar')renderGrammar();if(id==='mistakes')renderMistakes();if(id==='profile')renderProfile();if(id==='review')startReview();if(id==='textbook')renderTextbook();if(id==='listening')renderListeningTests();if(id==='knowledge')initKnowledge();
...
// ===== FAST AUTH BOOTSTRAP =====
// Fixes the login delay:
//  - ensureUserDoc runs admins + teachers lookups in parallel and only writes when needed.
//  - We DO NOT call ensureUserDoc twice anymore.
//  - initRosterGate reuses the profile from ensureUserDoc instead of re-reading users/{uid}.
onAuthStateChanged(auth,async u=>{
  user=u;
  if(u){
    try{
      let profile=await ensureUserDoc(u);
      let role=getCurrentRole();
      $('auth').classList.add('d-none');
      $('app').classList.remove('d-none');
      $('user').textContent=u.email||'';
      $('userName').textContent=profile?.name||u.displayName||'Tài khoản';
      $('avatar').textContent=(profile?.name||u.displayName||u.email||'U').trim().charAt(0).toUpperCase();
      let adminNav=document.querySelectorAll('[data-admin-nav]');
      adminNav.forEach(x=>x.classList.toggle('d-none',!isStaffRole(role)));

      const gate=await initRosterGate(u,role,profile);
      if(!gate){await signOut(auth);return}

      // Roster linking does not change role (admin/teacher come from config),
      // so we keep the role from the first ensureUserDoc and skip the redundant second read.
      if(isStaffRole(role))await loadAdminForUser(u);
      initKnowledge();
      initTeacherContent();
      if(!profile?.name)showNameModal(u);
      listen();
      renderProfile();
    }catch(e){
      console.error('Lỗi khởi tạo phiên đăng nhập:',e);
      $('authErr').textContent='Không thể khởi tạo tài khoản. Vui lòng thử lại.';
...
function toast(msg,type='success'){return window.appToast?window.appToast(msg,type):undefined}
...
const textbookVocab = [
  {"no":1,"unit":"Unit 1","word":"infection","pos":"N","meaning":"sự lây nhiễm","pattern":""},
  {"no":2,"unit":"Unit 1","word":"ingredient","pos":"N","meaning":"thành phần, nguyên liệu","pattern":""},
  {"no":3,"unit":"Unit 1","word":"life expectancy","pos":"N","meaning":"tuổi thọ","pattern":""},
  {"no":4,"unit":"Unit 1","word":"muscle","pos":"N","meaning":"cơ bắp","pattern":""},
  {"no":5,"unit":"Unit 1","word":"nutrient","pos":"N","meaning":"chất dinh dưỡng","pattern":""},
  {"no":6,"unit":"Unit 1","word":"press-up","pos":"N","meaning":"động tác chống đẩy","pattern":""},
  {"no":7,"unit":"Unit 1","word":"antibiotic","pos":"N","meaning":"thuốc kháng sinh","pattern":""},
  {"no":8,"unit":"Unit 1","word":"bacteria","pos":"N","meaning":"vi khuẩn","pattern":""},
  {"no":9,"unit":"Unit 1","word":"balanced","pos":"Adj","meaning":"cân bằng, điều độ","pattern":""},
  {"no":10,"unit":"Unit 1","word":"cut down on","pos":"Phrase","meaning":"cắt giảm","pattern":"cut down on + N"},
  {"no":11,"unit":"Unit 1","word":"diameter","pos":"N","meaning":"đường kính","pattern":""},
  {"no":12,"unit":"Unit 1","word":"disease","pos":"N","meaning":"bệnh","pattern":""},
  {"no":13,"unit":"Unit 1","word":"energy","pos":"N","meaning":"năng lượng","pattern":""},
  {"no":14,"unit":"Unit 1","word":"spread","pos":"N","meaning":"sự lây lan","pattern":""},
  {"no":15,"unit":"Unit 1","word":"examine","pos":"V","meaning":"kiểm tra, khám (sức khỏe)","pattern":""},
  {"no":16,"unit":"Unit 1","word":"fitness","pos":"N","meaning":"sức khỏe, thể lực","pattern":""},
  {"no":17,"unit":"Unit 1","word":"food poisoning","pos":"N","meaning":"ngộ độc thức ăn","pattern":""},
  {"no":18,"unit":"Unit 1","word":"star jump","pos":"N","meaning":"động tác nhảy dang tay chân","pattern":""},
  {"no":19,"unit":"Unit 1","word":"germ","pos":"N","meaning":"vi trùng","pattern":""},
  {"no":20,"unit":"Unit 1","word":"strength","pos":"N","meaning":"sức mạnh","pattern":""},
  {"no":21,"unit":"Unit 1","word":"give up","pos":"Phrase","meaning":"từ bỏ","pattern":"give up + N/V-ing"},
  {"no":22,"unit":"Unit 1","word":"suffer","pos":"V","meaning":"chịu đựng","pattern":"suffer from + illness/problem"},
  {"no":23,"unit":"Unit 1","word":"illness","pos":"N","meaning":"sự ốm đau","pattern":""},
  {"no":24,"unit":"Unit 1","word":"treatment","pos":"N","meaning":"cách điều trị","pattern":""},
  {"no":25,"unit":"Unit 1","word":"tuberculosis","pos":"N","meaning":"bệnh lao phổi","pattern":""},
  {"no":26,"unit":"Unit 1","word":"virus","pos":"N","meaning":"vi-rút","pattern":""},
  {"no":27,"unit":"Unit 1","word":"work out","pos":"Phrase","meaning":"tập thể dục","pattern":"work out"},
  {"no":28,"unit":"Unit 1","word":"properly","pos":"Adv","meaning":"một cách đúng, hợp lí","pattern":""},
  {"no":29,"unit":"Unit 1","word":"recipe","pos":"N","meaning":"công thức nấu ăn","pattern":""},
  {"no":30,"unit":"Unit 1","word":"regular","pos":"Adj","meaning":"đều đặn, thường xuyên","pattern":""},
  {"no":1,"unit":"Unit 2","word":"adapt","pos":"V","meaning":"thích nghi, thay đổi cho phù hợp","pattern":"adapt to + N/V-ing"},
  {"no":2,"unit":"Unit 2","word":"argument","pos":"N","meaning":"tranh luận, tranh cãi","pattern":""},
  {"no":3,"unit":"Unit 2","word":"characteristic","pos":"N","meaning":"đặc tính, đặc điểm","pattern":""},
  {"no":4,"unit":"Unit 2","word":"conflict","pos":"N","meaning":"sự xung đột, va chạm","pattern":""},
  {"no":5,"unit":"Unit 2","word":"curious","pos":"Adj","meaning":"tò mò, muốn tìm hiểu","pattern":""},
  {"no":6,"unit":"Unit 2","word":"digital native","pos":"N","meaning":"người được sinh ra trong thời đại công nghệ và Internet","pattern":""},
  {"no":7,"unit":"Unit 2","word":"experience","pos":"N/V","meaning":"trải nghiệm","pattern":""},
  {"no":8,"unit":"Unit 2","word":"extended family","pos":"N","meaning":"gia đình đa thế hệ, đại gia đình","pattern":""},
  {"no":9,"unit":"Unit 2","word":"freedom","pos":"N","meaning":"sự tự do","pattern":""},
  {"no":10,"unit":"Unit 2","word":"generation gap","pos":"N","meaning":"khoảng cách giữa các thế hệ","pattern":""},
  {"no":11,"unit":"Unit 2","word":"hire","pos":"V","meaning":"thuê nhân công, thuê người làm","pattern":""},
  {"no":12,"unit":"Unit 2","word":"honesty","pos":"N","meaning":"tính trung thực, tính chân thật","pattern":""},
  {"no":13,"unit":"Unit 2","word":"individualism","pos":"N","meaning":"chủ nghĩa cá nhân","pattern":""},
  {"no":14,"unit":"Unit 2","word":"influence","pos":"V","meaning":"gây ảnh hưởng","pattern":""},
  {"no":15,"unit":"Unit 2","word":"limit","pos":"V","meaning":"giới hạn, hạn chế","pattern":""},
  {"no":16,"unit":"Unit 2","word":"nuclear family","pos":"N","meaning":"gia đình hạt nhân, gồm 1–2 thế hệ","pattern":""},
  {"no":17,"unit":"Unit 2","word":"screentime","pos":"N","meaning":"thời gian sử dụng thiết bị điện tử","pattern":""},
  {"no":18,"unit":"Unit 2","word":"social media","pos":"N","meaning":"phương tiện truyền thông xã hội","pattern":""},
  {"no":19,"unit":"Unit 2","word":"value","pos":"N/V","meaning":"giá trị","pattern":""},
  {"no":20,"unit":"Unit 2","word":"view","pos":"N","meaning":"quan điểm","pattern":""},
  {"no":1,"unit":"Unit 3","word":"article","pos":"N","meaning":"bài báo","pattern":""},
  {"no":2,"unit":"Unit 3","word":"card reader","pos":"N","meaning":"thiết bị đọc thẻ","pattern":""},
  {"no":3,"unit":"Unit 3","word":"city dweller","pos":"N","meaning":"người dân thành phố","pattern":""},
  {"no":4,"unit":"Unit 3","word":"cycle path","pos":"N","meaning":"làn đường dành cho xe đạp","pattern":""},
  {"no":5,"unit":"Unit 3","word":"efficiently","pos":"Adv","meaning":"có hiệu quả","pattern":""},
  {"no":6,"unit":"Unit 3","word":"high-rise","pos":"Adj","meaning":"cao tầng, có nhiều tầng","pattern":""},
  {"no":7,"unit":"Unit 3","word":"infrastructure","pos":"N","meaning":"cơ sở hạ tầng","pattern":""},
  {"no":8,"unit":"Unit 3","word":"interact","pos":"V","meaning":"tương tác","pattern":""},
  {"no":9,"unit":"Unit 3","word":"liveable","pos":"Adj","meaning":"đáng sống","pattern":""},
  {"no":10,"unit":"Unit 3","word":"neighbourhood","pos":"N","meaning":"khu dân cư","pattern":""},
  {"no":11,"unit":"Unit 3","word":"operate","pos":"V","meaning":"vận hành","pattern":""},
  {"no":12,"unit":"Unit 3","word":"pedestrian","pos":"N","meaning":"người đi bộ","pattern":""},
  {"no":13,"unit":"Unit 3","word":"privacy","pos":"N","meaning":"sự riêng tư","pattern":""},
  {"no":14,"unit":"Unit 3","word":"roof garden","pos":"N","meaning":"vườn trên sân thượng","pattern":""},
  {"no":15,"unit":"Unit 3","word":"sense of community","pos":"Phrase","meaning":"ý thức cộng đồng","pattern":""},
  {"no":16,"unit":"Unit 3","word":"sensor","pos":"N","meaning":"cảm biến","pattern":""},
  {"no":17,"unit":"Unit 3","word":"skyscraper","pos":"N","meaning":"tòa nhà chọc trời","pattern":""},
  {"no":18,"unit":"Unit 3","word":"smart city","pos":"N","meaning":"thành phố thông minh","pattern":""},
  {"no":19,"unit":"Unit 3","word":"sustainable","pos":"Adj","meaning":"bền vững","pattern":""},
  {"no":20,"unit":"Unit 3","word":"urban centre","pos":"Phrase","meaning":"khu đô thị, trung tâm đô thị","pattern":""},
  {"no":1,"unit":"Unit 4","word":"apply for","pos":"V","meaning":"xin việc, ứng cử","pattern":"apply for + job/position"},
  {"no":2,"unit":"Unit 4","word":"celebration","pos":"N","meaning":"lễ kỉ niệm, lễ tổ chức","pattern":""},
  {"no":3,"unit":"Unit 4","word":"community","pos":"N","meaning":"cộng đồng","pattern":""},
  {"no":4,"unit":"Unit 4","word":"compliment","pos":"N","meaning":"lời khen","pattern":""},
  {"no":5,"unit":"Unit 4","word":"contribution","pos":"N","meaning":"sự đóng góp, cống hiến","pattern":""},
  {"no":6,"unit":"Unit 4","word":"cultural exchange","pos":"N","meaning":"sự trao đổi văn hóa","pattern":""},
  {"no":7,"unit":"Unit 4","word":"current","pos":"Adj","meaning":"hiện tại, đương thời","pattern":""},
  {"no":8,"unit":"Unit 4","word":"development","pos":"N","meaning":"sự phát triển","pattern":""},
  {"no":9,"unit":"Unit 4","word":"eye-opening","pos":"Adj","meaning":"mở mang tầm mắt","pattern":""},
  {"no":10,"unit":"Unit 4","word":"honour","pos":"V","meaning":"thể hiện sự kính trọng","pattern":""},
  {"no":11,"unit":"Unit 4","word":"issue","pos":"N","meaning":"vấn đề","pattern":""},
  {"no":12,"unit":"Unit 4","word":"leadership skills","pos":"Phrase","meaning":"kĩ năng lãnh đạo","pattern":""},
  {"no":13,"unit":"Unit 4","word":"live-stream","pos":"V","meaning":"phát sóng trực tuyến","pattern":""},
  {"no":14,"unit":"Unit 4","word":"politics","pos":"N","meaning":"chính trị","pattern":""},
  {"no":15,"unit":"Unit 4","word":"promote","pos":"V","meaning":"thúc đẩy, khuyến mãi, quảng bá","pattern":""},
  {"no":16,"unit":"Unit 4","word":"proposal","pos":"N","meaning":"lời/ bản đề xuất","pattern":""},
  {"no":17,"unit":"Unit 4","word":"propose","pos":"V","meaning":"đề xuất","pattern":""},
  {"no":18,"unit":"Unit 4","word":"qualify","pos":"V","meaning":"đủ tiêu chuẩn, đủ khả năng","pattern":""},
  {"no":19,"unit":"Unit 4","word":"region","pos":"N","meaning":"vùng","pattern":""},
  {"no":20,"unit":"Unit 4","word":"relation","pos":"N","meaning":"mối quan hệ","pattern":""},
  {"no":21,"unit":"Unit 4","word":"represent","pos":"V","meaning":"đại diện, tượng trưng","pattern":""},
  {"no":22,"unit":"Unit 4","word":"representative","pos":"N","meaning":"người đại diện","pattern":""},
  {"no":23,"unit":"Unit 4","word":"strengthen","pos":"V","meaning":"tăng cường, đẩy mạnh","pattern":""},
  {"no":24,"unit":"Unit 4","word":"support","pos":"V","meaning":"hỗ trợ","pattern":""},
  {"no":25,"unit":"Unit 4","word":"take part in","pos":"Phrase","meaning":"tham gia","pattern":"take part in + N"},
  {"no":26,"unit":"Unit 4","word":"volunteer","pos":"N/V","meaning":"tình nguyện, tình nguyện viên","pattern":""},
  {"no":27,"unit":"Unit 4","word":"youth","pos":"N","meaning":"tuổi trẻ","pattern":""},
  {"no":1,"unit":"Unit 5","word":"atmosphere","pos":"N","meaning":"khí quyển","pattern":""},
  {"no":2,"unit":"Unit 5","word":"balance","pos":"N","meaning":"sự cân bằng","pattern":""},
  {"no":3,"unit":"Unit 5","word":"carbon dioxide","pos":"N","meaning":"khí cacbonic (CO₂)","pattern":""},
  {"no":4,"unit":"Unit 5","word":"coal","pos":"N","meaning":"than đá","pattern":""},
  {"no":5,"unit":"Unit 5","word":"consequence","pos":"N","meaning":"hậu quả, kết quả","pattern":""},
  {"no":6,"unit":"Unit 5","word":"cut down","pos":"Phrase","meaning":"chặt, đốn (cây)","pattern":"cut down + tree"},
  {"no":7,"unit":"Unit 5","word":"deforestation","pos":"N","meaning":"sự phá rừng","pattern":""},
  {"no":8,"unit":"Unit 5","word":"emission","pos":"N","meaning":"sự phát thải","pattern":""},
  {"no":9,"unit":"Unit 5","word":"environment","pos":"N","meaning":"môi trường","pattern":""},
  {"no":10,"unit":"Unit 5","word":"farming","pos":"N","meaning":"nghề nông","pattern":""},
  {"no":11,"unit":"Unit 5","word":"farmland","pos":"N","meaning":"đất chăn nuôi/trồng trọt","pattern":""},
  {"no":12,"unit":"Unit 5","word":"fossil fuel","pos":"N","meaning":"nhiên liệu hóa thạch","pattern":""},
  {"no":13,"unit":"Unit 5","word":"global warming","pos":"N","meaning":"sự nóng lên toàn cầu","pattern":""},
  {"no":14,"unit":"Unit 5","word":"heat-trapping","pos":"Adj","meaning":"giữ nhiệt","pattern":""},
  {"no":15,"unit":"Unit 5","word":"human activity","pos":"N","meaning":"hoạt động của con người","pattern":""},
  {"no":16,"unit":"Unit 5","word":"leaflet","pos":"N","meaning":"tờ rơi","pattern":""},
  {"no":17,"unit":"Unit 5","word":"methane","pos":"N","meaning":"khí mê-tan (CH₄)","pattern":""},
  {"no":18,"unit":"Unit 5","word":"pollutant","pos":"N","meaning":"chất gây ô nhiễm","pattern":""},
  {"no":19,"unit":"Unit 5","word":"release","pos":"V","meaning":"thải ra, giải phóng","pattern":""},
  {"no":20,"unit":"Unit 5","word":"renewable","pos":"Adj","meaning":"tái tạo, có thể tái tạo","pattern":""},
  {"no":21,"unit":"Unit 5","word":"sealevel","pos":"N","meaning":"mực nước biển","pattern":""},
  {"no":22,"unit":"Unit 5","word":"soil","pos":"N","meaning":"đất trồng","pattern":""},
  {"no":23,"unit":"Unit 5","word":"soot","pos":"N","meaning":"muội, bồ hóng","pattern":""},
  {"no":24,"unit":"Unit 5","word":"temperature","pos":"N","meaning":"nhiệt độ","pattern":""},
  {"no":25,"unit":"Unit 5","word":"waste","pos":"N","meaning":"rác, chất thải","pattern":""},
  {"no":1,"unit":"Unit 6","word":"ancient","pos":"Adj","meaning":"cổ kính","pattern":""},
  {"no":2,"unit":"Unit 6","word":"appreciate","pos":"V","meaning":"hiểu rõ giá trị, đánh giá cao","pattern":""},
  {"no":3,"unit":"Unit 6","word":"performing arts","pos":"N","meaning":"nghệ thuật biểu diễn","pattern":""},
  {"no":4,"unit":"Unit 6","word":"citadel","pos":"N","meaning":"thành trì","pattern":""},
  {"no":5,"unit":"Unit 6","word":"complex","pos":"N","meaning":"quần thể, tổ hợp","pattern":""},
  {"no":6,"unit":"Unit 6","word":"crowdfunding","pos":"N","meaning":"việc quyên góp, huy động vốn từ cộng đồng","pattern":""},
  {"no":7,"unit":"Unit 6","word":"festive","pos":"Adj","meaning":"thuộc về ngày lễ, có không khí lễ hội","pattern":""},
  {"no":8,"unit":"Unit 6","word":"fine","pos":"N","meaning":"tiền phạt","pattern":""},
  {"no":9,"unit":"Unit 6","word":"folk","pos":"Adj","meaning":"thuộc về dân gian","pattern":""},
  {"no":10,"unit":"Unit 6","word":"heritage","pos":"N","meaning":"di sản","pattern":""},
  {"no":11,"unit":"Unit 6","word":"historic","pos":"Adj","meaning":"quan trọng, có giá trị lịch sử","pattern":""},
  {"no":12,"unit":"Unit 6","word":"historical","pos":"Adj","meaning":"thuộc về lịch sử, mang tính lịch sử","pattern":""},
  {"no":13,"unit":"Unit 6","word":"imperial","pos":"Adj","meaning":"thuộc về hoàng tộc","pattern":""},
  {"no":14,"unit":"Unit 6","word":"landscape","pos":"N","meaning":"phong cảnh","pattern":""},
  {"no":15,"unit":"Unit 6","word":"limestone","pos":"N","meaning":"đá vôi","pattern":""},
  {"no":16,"unit":"Unit 6","word":"monument","pos":"N","meaning":"lăng mộ, đài kỉ niệm, công trình tưởng niệm","pattern":""},
  {"no":17,"unit":"Unit 6","word":"preserve","pos":"V","meaning":"bảo tồn","pattern":""},
  {"no":18,"unit":"Unit 6","word":"restore","pos":"V","meaning":"khôi phục, sửa lại","pattern":""},
  {"no":19,"unit":"Unit 6","word":"state","pos":"N","meaning":"hiện trạng, tình trạng","pattern":""},
  {"no":20,"unit":"Unit 6","word":"temple","pos":"N","meaning":"đền, miếu","pattern":""},
  {"no":21,"unit":"Unit 6","word":"trending","pos":"Adj","meaning":"theo xu hướng","pattern":""},
  {"no":22,"unit":"Unit 6","word":"valley","pos":"N","meaning":"thung lũng","pattern":""},
  {"no":1,"unit":"Unit 7","word":"academic","pos":"Adj","meaning":"có tính chất học thuật, liên quan tới học tập","pattern":""},
  {"no":2,"unit":"Unit 7","word":"apprenticeship","pos":"N","meaning":"thời gian học nghề, học việc","pattern":""},
  {"no":3,"unit":"Unit 7","word":"bachelor’s degree","pos":"N","meaning":"bằng cử nhân","pattern":""},
  {"no":4,"unit":"Unit 7","word":"brochure","pos":"N","meaning":"ấn phẩm quảng cáo, giới thiệu","pattern":""},
  {"no":5,"unit":"Unit 7","word":"doctorate","pos":"N","meaning":"bằng tiến sĩ","pattern":""},
  {"no":6,"unit":"Unit 7","word":"entrance exam","pos":"N","meaning":"kì thi đầu vào","pattern":""},
  {"no":7,"unit":"Unit 7","word":"formal","pos":"Adj","meaning":"chính quy, có hệ thống","pattern":""},
  {"no":8,"unit":"Unit 7","word":"graduation","pos":"N","meaning":"khi tốt nghiệp, lễ tốt nghiệp","pattern":""},
  {"no":9,"unit":"Unit 7","word":"higher education","pos":"N","meaning":"giáo dục đại học","pattern":""},
  {"no":10,"unit":"Unit 7","word":"institution","pos":"N","meaning":"cơ sở, viện (đào tạo)","pattern":""},
  {"no":11,"unit":"Unit 7","word":"manage","pos":"V","meaning":"cố gắng, làm được việc gì đó","pattern":""},
  {"no":12,"unit":"Unit 7","word":"master’s degree","pos":"N","meaning":"bằng thạc sĩ","pattern":""},
  {"no":13,"unit":"Unit 7","word":"mechanic","pos":"N","meaning":"thợ cơ khí","pattern":""},
  {"no":14,"unit":"Unit 7","word":"professional","pos":"Adj","meaning":"chuyên nghiệp, nhà nghề","pattern":""},
  {"no":15,"unit":"Unit 7","word":"qualification","pos":"N","meaning":"trình độ chuyên môn, văn bằng","pattern":""},
  {"no":16,"unit":"Unit 7","word":"school-leaver","pos":"N","meaning":"học sinh tốt nghiệp trung học phổ thông","pattern":""},
  {"no":17,"unit":"Unit 7","word":"sixth-form college","pos":"N","meaning":"trường cho học sinh 16–19 tuổi, tập trung vào các trình độ A-levels để chuẩn bị vào đại học","pattern":""},
  {"no":18,"unit":"Unit 7","word":"vocational school","pos":"N","meaning":"trường dạy nghề","pattern":""},
  {"no":1,"unit":"Unit 8","word":"achieve","pos":"V","meaning":"đạt được, giành được","pattern":""},
  {"no":2,"unit":"Unit 8","word":"carry out","pos":"Phrase","meaning":"tiến hành","pattern":""},
  {"no":3,"unit":"Unit 8","word":"combine","pos":"V","meaning":"kết hợp","pattern":""},
  {"no":4,"unit":"Unit 8","word":"come up with","pos":"Phrase","meaning":"nghĩ ra, nảy ra","pattern":"come up with + idea/solution"},
  {"no":5,"unit":"Unit 8","word":"confidence","pos":"N","meaning":"sự tự tin","pattern":""},
  {"no":6,"unit":"Unit 8","word":"confident","pos":"Adj","meaning":"tự tin","pattern":""},
  {"no":7,"unit":"Unit 8","word":"deal with","pos":"Phrase","meaning":"giải quyết, đối phó","pattern":"deal with + problem/person"},
  {"no":8,"unit":"Unit 8","word":"decision-making skills","pos":"Phrase","meaning":"kĩ năng đưa ra quyết định","pattern":""},
  {"no":9,"unit":"Unit 8","word":"get around","pos":"Phrase","meaning":"đi lại","pattern":""},
  {"no":10,"unit":"Unit 8","word":"get into the habit of","pos":"Phrase","meaning":"tạo thói quen","pattern":"get into the habit of + V-ing"},
  {"no":11,"unit":"Unit 8","word":"independence","pos":"N","meaning":"sự độc lập","pattern":""},
  {"no":12,"unit":"Unit 8","word":"independent","pos":"Adj","meaning":"độc lập, không lệ thuộc","pattern":""},
  {"no":13,"unit":"Unit 8","word":"learner","pos":"N","meaning":"người học","pattern":""},
  {"no":14,"unit":"Unit 8","word":"learning goal","pos":"Phrase","meaning":"mục tiêu học tập","pattern":""},
  {"no":15,"unit":"Unit 8","word":"life skill","pos":"N","meaning":"kĩ năng sống","pattern":""},
  {"no":16,"unit":"Unit 8","word":"make use of","pos":"Phrase","meaning":"tận dụng","pattern":"make use of + N"},
  {"no":17,"unit":"Unit 8","word":"manage","pos":"V","meaning":"quản lí","pattern":""},
  {"no":18,"unit":"Unit 8","word":"measure","pos":"V","meaning":"đo","pattern":""},
  {"no":19,"unit":"Unit 8","word":"money-management skills","pos":"Phrase","meaning":"kĩ năng quản lí tiền","pattern":""},
  {"no":20,"unit":"Unit 8","word":"remove","pos":"V","meaning":"lấy ra, loại bỏ","pattern":""},
  {"no":21,"unit":"Unit 8","word":"responsibility","pos":"N","meaning":"sự chịu trách nhiệm, trách nhiệm","pattern":""},
  {"no":22,"unit":"Unit 8","word":"responsible","pos":"Adj","meaning":"có trách nhiệm","pattern":""},
  {"no":23,"unit":"Unit 8","word":"rice cooker","pos":"N","meaning":"nồi cơm điện","pattern":""},
  {"no":24,"unit":"Unit 8","word":"self-motivated","pos":"Adj","meaning":"có động lực","pattern":""},
  {"no":25,"unit":"Unit 8","word":"self-study","pos":"N","meaning":"sự tự học","pattern":""},
  {"no":26,"unit":"Unit 8","word":"time-management skills","pos":"Phrase","meaning":"kĩ năng quản lí thời gian","pattern":""},
  {"no":1,"unit":"Unit 9","word":"admit","pos":"V","meaning":"thừa nhận","pattern":""},
  {"no":2,"unit":"Unit 9","word":"alcohol","pos":"N","meaning":"đồ uống có cồn (rượu, bia...)","pattern":""},
  {"no":3,"unit":"Unit 9","word":"anxiety","pos":"N","meaning":"sự lo lắng","pattern":""},
  {"no":4,"unit":"Unit 9","word":"ashamed","pos":"Adj","meaning":"xấu hổ","pattern":""},
  {"no":5,"unit":"Unit 9","word":"awareness","pos":"N","meaning":"nhận thức","pattern":""},
  {"no":6,"unit":"Unit 9","word":"body shaming","pos":"N","meaning":"sự chê bai ngoại hình của người khác","pattern":""},
  {"no":7,"unit":"Unit 9","word":"bully","pos":"V","meaning":"bắt nạt","pattern":""},
  {"no":8,"unit":"Unit 9","word":"campaign","pos":"N","meaning":"chiến dịch","pattern":""},
  {"no":9,"unit":"Unit 9","word":"crime","pos":"N","meaning":"tội phạm","pattern":""},
  {"no":10,"unit":"Unit 9","word":"cyberbullying","pos":"N","meaning":"bắt nạt trên mạng","pattern":""},
  {"no":11,"unit":"Unit 9","word":"depression","pos":"N","meaning":"sự trầm cảm","pattern":""},
  {"no":12,"unit":"Unit 9","word":"hang out","pos":"Phrase","meaning":"đi chơi","pattern":""},
  {"no":13,"unit":"Unit 9","word":"lie","pos":"N","meaning":"lời nói dối","pattern":""},
  {"no":14,"unit":"Unit 9","word":"make fun of","pos":"Phrase","meaning":"trêu chọc, chế giễu","pattern":""},
  {"no":15,"unit":"Unit 9","word":"offensive","pos":"Adj","meaning":"gây xúc phạm","pattern":""},
  {"no":16,"unit":"Unit 9","word":"overpopulation","pos":"N","meaning":"sự quá tải dân số","pattern":""},
  {"no":17,"unit":"Unit 9","word":"peer pressure","pos":"N","meaning":"áp lực từ bạn bè","pattern":""},
  {"no":18,"unit":"Unit 9","word":"physical","pos":"Adj","meaning":"về mặt thể chất","pattern":""},
  {"no":19,"unit":"Unit 9","word":"poverty","pos":"N","meaning":"sự nghèo đói","pattern":""},
  {"no":20,"unit":"Unit 9","word":"self-confidence","pos":"N","meaning":"sự tự tin vào bản thân","pattern":""},
  {"no":21,"unit":"Unit 9","word":"skip","pos":"V","meaning":"trốn, bỏ","pattern":""},
  {"no":22,"unit":"Unit 9","word":"stand up to","pos":"Phrase","meaning":"đứng lên chống lại","pattern":"stand up to + N"},
  {"no":23,"unit":"Unit 9","word":"struggle","pos":"V","meaning":"đấu tranh","pattern":""},
  {"no":24,"unit":"Unit 9","word":"the odd one out","pos":"Phrase","meaning":"kẻ/người khác biệt","pattern":""},
  {"no":25,"unit":"Unit 9","word":"the poverty line","pos":"N","meaning":"mức nghèo đói","pattern":""},
  {"no":26,"unit":"Unit 9","word":"verbal","pos":"Adj","meaning":"bằng lời nói","pattern":""},
  {"no":27,"unit":"Unit 9","word":"victim","pos":"N","meaning":"nạn nhân","pattern":""},
  {"no":28,"unit":"Unit 9","word":"violent","pos":"Adj","meaning":"sử dụng vũ lực, bạo lực","pattern":""},
  {"no":1,"unit":"Unit 10","word":"biodiversity","pos":"N","meaning":"đa dạng sinh học","pattern":""},
  {"no":2,"unit":"Unit 10","word":"conservation","pos":"N","meaning":"sự bảo tồn thiên nhiên","pattern":""},
  {"no":3,"unit":"Unit 10","word":"coral reef","pos":"N","meaning":"rạn san hô","pattern":""},
  {"no":4,"unit":"Unit 10","word":"delta","pos":"N","meaning":"đồng bằng","pattern":""},
  {"no":5,"unit":"Unit 10","word":"destroy","pos":"V","meaning":"phá hủy","pattern":""},
  {"no":6,"unit":"Unit 10","word":"ecosystem","pos":"N","meaning":"hệ sinh thái","pattern":""},
  {"no":7,"unit":"Unit 10","word":"endangered","pos":"Adj","meaning":"bị nguy hiểm","pattern":""},
  {"no":8,"unit":"Unit 10","word":"fauna","pos":"N","meaning":"động vật","pattern":""},
  {"no":9,"unit":"Unit 10","word":"flora","pos":"N","meaning":"thực vật","pattern":""},
  {"no":10,"unit":"Unit 10","word":"food chain","pos":"N","meaning":"chuỗi thức ăn","pattern":""},
  {"no":11,"unit":"Unit 10","word":"green","pos":"Adj","meaning":"xanh","pattern":""},
  {"no":12,"unit":"Unit 10","word":"habitat","pos":"N","meaning":"khu vực sống","pattern":""},
  {"no":13,"unit":"Unit 10","word":"living things","pos":"N","meaning":"các sinh vật sống","pattern":""},
  {"no":14,"unit":"Unit 10","word":"mammal","pos":"N","meaning":"động vật có vú","pattern":""},
  {"no":15,"unit":"Unit 10","word":"national park","pos":"N","meaning":"rừng quốc gia","pattern":""},
  {"no":16,"unit":"Unit 10","word":"native","pos":"Adj","meaning":"tự nhiên, bản địa","pattern":""},
  {"no":17,"unit":"Unit 10","word":"natural resources","pos":"N","meaning":"tài nguyên thiên nhiên","pattern":""},
  {"no":18,"unit":"Unit 10","word":"pangolin","pos":"N","meaning":"con tê tê","pattern":""},
  {"no":19,"unit":"Unit 10","word":"resource","pos":"N","meaning":"nguồn lực","pattern":""},
  {"no":20,"unit":"Unit 10","word":"species","pos":"N","meaning":"loài","pattern":""},
  {"no":21,"unit":"Unit 10","word":"tropical forest","pos":"N","meaning":"rừng nhiệt đới","pattern":""},
  {"no":22,"unit":"Unit 10","word":"wildlife","pos":"N","meaning":"động vật hoang dã","pattern":""}
];
...
const getExerciseBank=()=>typeof exerciseBank!=='undefined'&&Array.isArray(exerciseBank)?exerciseBank:[];
const getListeningBank=()=>typeof listeningBank!=='undefined'&&Array.isArray(listeningBank)?listeningBank:[];
const getListeningSources=()=>typeof listeningSources!=='undefined'&&listeningSources?listeningSources:{};
function exerciseItems(unit,count){
  if(typeof unit!=='string'||!unit.trim())return[];
  const safeCount=Number.isFinite(Number(count))?Math.max(1,Math.floor(Number(count))):3;
  const bank=getExerciseBank();
  const entry=bank.find(x=>x&&x.unit===unit);
  const sections=entry?.sections;
  if(!sections||typeof sections!=='object')return[];
  const out=[];
  const addItems=(kind,source,maxCount)=>{
...
function renderExerciseItem(item,n){
  if(!item||typeof item!=='object')return'<div class="exercise-card"><div class="exercise-feedback wrong">Không thể hiển thị câu hỏi này.</div></div>';
  const typeLabels={mcq:'Choose the best answer — ABCD',two:'Choose ONE of the TWO',form:'Complete the sentence — correct form',rewrite:'Rewrite without changing meaning',reading:'Reading comprehension',closest:'Closest meaning',opposite:'Opposite meaning'};
  const label=typeLabels[item.kind]||'Exercise';
  const prompt=esc(item.prompt||'Câu hỏi chưa có nội dung.');
  const options=Array.isArray(item.options)?item.options:[];
  if(item.kind==='form'||item.kind==='rewrite'){
    return `<div class="exercise-card"><div class="exercise-label">${label}</div><h4>${n+1}. ${prompt}</h4><input id="exText" class="form-control form-control-lg" autocomplete="off" placeholder="${item.kind==='form'?'Type the correct form…':'Write the new sentence…'}"></div>`;
  }
  if(options.length<2)return'<div class="exercise-card"><div class="exercise-feedback wrong">Câu hỏi không hợp lệ — đã bỏ đáp án thiếu.</div></div>';
  const letters='ABCD';
  const optionHtml=options.map((x,i)=>{
    const text=String(x??'');
    return `<label class="exercise-option"><input type="radio" name="exAnswer" value="${i}"><span><b>${letters[i]||String(i+1)}.</b> ${esc(text)}</span></label>`;
  }).join('');
  if(item.kind==='reading'){
    const unit=exerciseSession?.unit||'';
    const bank=getExerciseBank();
    const entry=bank.find(x=>x&&x.unit===unit);
    const passage=entry?.sections?.reading?.text;
    const passageHtml=typeof passage==='string'&&passage.trim()?highlightSavedReading(passage,unit):'';
    return `<div class="exercise-card"><div class="exercise-label">${label}</div><div class="reading-tools"><div class="reading-tool-hint">💡 Bôi đen một từ/cụm từ trong bài đọc để <b>tô đậm · dịch · thêm vào Từ mới</b>.</div><div class="reading-selection-result d-none" id="readingSelectionResult"></div></div>${passageHtml?`<div class="reading-passage" data-reading-unit="${esc(unit)}">${passageHtml}</div>`:''}<h4>${n+1}. ${prompt}</h4><div class="option-grid">${optionHtml}</div></div>`;
  }
  const meaningInstruction=item.kind==='closest'?'Choose the closest meaning.':item.kind==='opposite'?'Choose the opposite meaning.':'';
  return `<div class="exercise-card"><div class="exercise-label">${label}</div><h4>${n+1}. ${prompt}</h4>${meaningInstruction?`<p class="muted">${meaningInstruction}</p>`:''}<div class="option-grid">${optionHtml}</div></div>`;
}
...
function correctExercise(item,val){
  if(!item||typeof item!=='object')return false;
  if(item.kind==='form'||item.kind==='rewrite')return normalizeAnswer(val)===normalizeAnswer(item.answer||'');
  const correctIndex=Number(item.correctIndex);
  const selected=Number(val);
  return Number.isInteger(correctIndex)&&Number.isInteger(selected)&&selected===correctIndex;
}
function answerText(item){
  if(!item||!Array.isArray(item.options)||item.options.length===0)return'(không có đáp án)';
  const index=Number(item.correctIndex);
  if(!Number.isInteger(index)||index<0||index>=item.options.length)return'(không có đáp án)';
  return String(item.options[index]??'(không có đáp án)');
}
async function checkExerciseAnswer(){
  const s=exerciseSession;
  if(!s||!Array.isArray(s.items)||!s.items.length||s.index<0||s.index>=s.items.length)return;
  const item=s.items[s.index];
  if(!item||s.checking)return;
  let val='';
  if(item.kind==='form'||item.kind==='rewrite')val=$('exText')?.value||'';
  else val=document.querySelector('input[name="exAnswer"]:checked')?.value||'';
  if(!String(val).trim()){toast('Chọn/nhập đáp án trước nhé.','error');return;}
  s.checking=true;
  const btn=$('exerciseCheck');
  if(btn){btn.disabled=true;btn.innerHTML='<span class="spinner-border spinner-border-sm me-1"></span>Đang kiểm tra…';}
  try{
    const ok=correctExercise(item,val);
    if(!s.answered){if(ok)s.score++;s.answered=true;}
    s.lastAnswer=val;
    const fb=$('exerciseFeedback');
    if(!fb)return;
    fb.classList.remove('d-none');
    fb.className='exercise-feedback '+(ok?'correct':'wrong');
    ...
    if(!ok&&user&&!s.loggedWrong?.[s.index]){
      s.loggedWrong=s.loggedWrong||{};s.loggedWrong[s.index]=true;
      try{
        const prompt=String(item.prompt||'Câu hỏi bài tập');
        const rule=String(item.explain||`Đáp án đúng: ${answer}`);
    ...
    const nav=$('exerciseArea')?.querySelector('.exercise-nav');
    if(!nav)return;
    if(s.index<s.items.length-1){
      const n=document.createElement('button');
      n.id='exerciseNext';n.className='btn btn-primary';n.textContent='Tiếp →';
      n.onclick=()=>{s.index++;s.answered=false;s.checking=false;renderExerciseCard();};
      nav.appendChild(n);
    }else{
      const n=document.createElement('button');
      n.id='exerciseFinish';n.className='btn btn-success';n.textContent='Xem kết quả';n.onclick=finishExercise;
      nav.appendChild(n);
    }
  }catch(e){
...
function startExercise(){const unit=$('exerciseUnit').value,count=Number($('exerciseCount').value||3);exerciseSession={unit,items:exerciseItems(unit,count),index:0,score:0,answered:false,loggedWrong:{}};$('exerciseSetup').classList.add('d-none');$('exerciseArea').classList.remove('d-none');renderExerciseCard()}
...
function listeningTests(unit){return getListeningBank().filter(x=>x.unit===unit)}
