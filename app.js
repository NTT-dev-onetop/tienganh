...
onAuthStateChanged(auth,async u=>{user=u;if(u){try{const profile=await ensureUserDoc(u);const role=getCurrentRole();$('auth').classList.add('d-none');$('app').classList.remove('d-none');$('user').textContent=u.email||'';$('userName').textContent=profile?.name||u.displayName||'Tài khoản';$('avatar').textContent=(profile?.name||u.displayName||u.email||'U').trim().charAt(0).toUpperCase();const adminNav=document.querySelectorAll('[data-admin-nav]');adminNav.forEach(x=>x.classList.toggle('d-none',!isStaffRole(role)));const gate=await initRosterGate(u,role);if(isStaffRole(role))await loadAdminForUser(u);initKnowledge();initTeacherContent();if(!gate){await signOut(auth);return}if(!profile?.name)showNameModal(u);listen();renderProfile()}catch(e){console.error('Lỗi khởi tạo phiên đăng nhập:',e);$('authErr').textContent='Không thể khởi tạo tài khoản. Vui lòng thử lại.';$('authErr').classList.remove('d-none');await signOut(auth)}}else{clearCurrentRole();$('auth').classList.remove('d-none');$('app').classList.add('d-none');if(unsub){unsub();unsub=null}stopKnowledge();stopTeacherContent()}});
